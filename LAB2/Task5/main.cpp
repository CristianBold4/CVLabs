...
void plot_histogram(Mat src, int nbins, const String &window)
{
    int h_width = 400;
    int h_height = 512;
    ...
    for (int i = 0; i < nbins; i++)
    {
        rectangle(histogram_image, Point(2 * i, histogram_image.rows - normalized_hist.at<float>(i)),
                  Point(2 * (i+1), histogram_image.rows), Scalar(255));
    }

    namedWindow(window, WINDOW_NORMAL);
    imshow(window, histogram_image);
...
int main(int argc, char** argv)
{
    ...
    plot_histogram(gray_src, 256, "Source Histogram");
    waitKey();

    //CALCULATING EQUALIZED IMAGE
    Mat equalized_src;
    equalizeHist( gray_src, equalized_src );

	namedWindow("Equalized Image", WINDOW_NORMAL);
    imshow( "Equalized Image", equalized_src );
    waitKey();

	//CALCULATING AND PLOTTING EQUALIZED HISTOGRAM
    plot_histogram(equalized_src, 256, "Equalized Histogram");
    waitKey();
